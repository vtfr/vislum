...
pub fn derive_node_impl(input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    let input_constructor = inputs.iter().enumerate().map(|(index, input)| {
        let ident = input.ident;
        let ty = input.ty;
    ...
    let input_indexes = (0..inputs_len).collect::<Vec<_>>();
    let input_idents = inputs.iter().map(|input| input.ident).collect::<Vec<_>>();

    let outputs_len = outputs.len();
    let output_indexes = (0..outputs_len).collect::<Vec<_>>();
    ...
    let result = quote! {
        ...
        #[automatically_derived]
        impl vislum_op::introspect::Introspect for #ident {
            fn inputs_len(&self) -> usize {
                #inputs_len
            }

            fn get_input(&self, input_id: vislum_op::node::InputId) -> Option<&dyn vislum_op::introspect::IntrospectInput> {
                match input_id {
            ...
            fn get_input_mut(&mut self, input_id: vislum_op::node::InputId) -> Option<&mut dyn vislum_op::introspect::IntrospectInput> {
                match input_id {
            ...
            fn get_output_value(&self, output_id: vislum_op::node::OutputId) -> Option<TaggedValue> {
                match output_id {
            ...
            fn get_input_definition(&self, input_id: vislum_op::node::InputId) -> Option<&vislum_op::node_type::InputDefinition> {
                match input_id {
                    #(#input_indexes => Some(&self.#input_idents),)*
                    _ => None,
                }
            ...
            fn get_output_definition(&self, output_id: vislum_op::node::OutputId) -> Option<&vislum_op::node_type::OutputDefinition> {
                match output_id {
