...
impl NodeGraphSystem {
    ...
    pub fn add_node(&mut self, node_type_id: &str) -> NodeId {
        let node_type = self.node_type_registry.get(node_type_id)
            .unwrap();
    ...
    pub fn assign_constant(&mut self, node_id: NodeId, input_id: InputId, value: TaggedValue) -> Result<(), GraphError> {
        self.graph.assign_constant(node_id, input_id, value)?;
        self.needs_recompilation = true;
        Ok(())
    }

    pub fn connect(&mut self, node_id: NodeId, input_id: InputId, placement: ConnectionPlacement, connection: Connection) -> Result<(), GraphError> {
        self.graph.connect(node_id, input_id, placement, connection)?;
        self.needs_recompilation = true;
        Ok(())
    }

    pub fn eval(&mut self, ctx: &EvalContext, entry_point: NodeId) -> Result<&NodeRef, NodeGraphSystemError> {
        ...
        match &self.compiled_nodes {
            Some(Ok(compiled_nodes)) => {
                let entry_point = compiled_nodes.get(&entry_point)
